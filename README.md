...
- **A minimal local store.** Persists only a child profile (date-of-birth so age never goes stale), explicitly-saved durable constraints, and experiment state — **never raw sleep logs**. Stored locally on your machine, never uploaded; see **[DATA_HANDLING.md](DATA_HANDLING.md)** for exactly what's kept, how connected-log access works, and how to inspect or delete it.
