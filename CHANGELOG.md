...
- **Performance: cut time-to-first-reply by removing serial reads from the common turn.**
  ...
  (5) Citing a claim no longer loads the ~64KB `claims.yaml` / ~30KB `sources.yaml` into the
  conversation: a generated `knowledge/claims-index.md` (~8KB, one scannable line per claim)
  lets the agent pick which claims to cite, then `lullsense-cite <claim_id|source_id>` fetches
  only those full entries. The index is generated by `scripts/build_knowledge_index.py` and
  CI fails if it drifts from `claims.yaml`; A_safety citations stay embedded inline in
  `safety-triage.md`. (6) The enumerated "Cross-References" footer menus were removed from the
  reference files (persona, prediction, interventions, intake, practice-map, sleep-training) —
